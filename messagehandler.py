...
def messageHandler(update: Update, context: CallbackContext):
    if update.message.text!="":
        if dictionary["job"+str(update.effective_chat.id)]==1:
            ...
            context.bot.send_message(chat_id=update.effective_chat.id, reply_markup=InlineKeyboardMarkup(buttons), text="so you did NOT forget anything from your yesterday's progress?")
        ...
        if dictionary["job"+str(update.effective_chat.id)]==2:
            a="today's plan : "+str(update.message.text)+"\n"+"time when he answered: "+str(datetime.now() - timedelta(hours=9))[:16]+"\n"+ "name: "+str(update.effective_chat.full_name)
        ...
        if dictionary["job"+str(update.effective_chat.id)]==3:
            ...
            context.bot.send_message(chat_id=update.effective_chat.id, reply_markup=InlineKeyboardMarkup(buttons), text="soo main part is that all?")
            if tel_resp.status_code==200:
                print("sent2")
            else:
                print("error2")
